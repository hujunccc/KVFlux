...
单 token decode 追加与非连续物理页的例子见 [Milestone 4：Dynamic Sequence Growth](v2_dynamic_sequence_growth.md)。当前仅预留逻辑 token 位置并管理物理页编号，不写入 K/V 数据，也不实现共享尾块的写时复制。真实 GPU 地址、内容写入与计算调度属于后续阶段。
